...
func TestCosineSimilarity(t *testing.T) {
	...
	_, err = client.CosineSimilarity([]float64{0, 0}, []float64{1, 2})
	assert.NotNil(t, err, "Expected error for zero vector")

	// Test case: cosine similarity of identical vectors
	result, err := client.CosineSimilarity([]float64{1, 2, 3}, []float64{1, 2, 3})
	...
	result, err = client.CosineSimilarity([]float64{1, 0}, []float64{0, 1})
	...
	result, err = client.CosineSimilarity([]float64{1, 2, 3}, []float64{4, 5, 6})
