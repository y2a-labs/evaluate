...
func (rs Resources) RegisterAgentRoutes(s *fuego.Server) {
	...
	fuego.Get(AgentGroup, "/{id}/conversations", rs.agentConversationsPage)
...
func (rs Resources) agentConversationsPage(c fuego.ContextNoBody) (fuego.HTML, error) {
	id := c.PathParam("id")
	agent := &models.Agent{BaseModel: models.BaseModel{ID: id}}
	tx := rs.Service.Db.
		Preload("Conversations", func(db *gorm.DB) *gorm.DB {
			return db.Where("is_test = ?", false).Order("created_at DESC").Limit(10)
		}).First(agent)

	if tx.Error != nil {
		return "", tx.Error
	}
	return c.Render("pages/agent-conversations.page.html", agent)
}
